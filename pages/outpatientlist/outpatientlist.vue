<template>
	<view class="content"><view class="search-inp">
		...
		<view class="index-select">
			<view class="item">
				<picker class="picker-item" @click="classBool = !classBool" @cancel="classBool = !classBool" @change="bindSectionChange" :value="index" :range="sectionNameList">{{sectionList[index].name}}
	...
	export default {
		...
		methods: {
			...
			getList(){
				...
				params.searchType = 2;
				tools.request("/api/index/searchDoctorOrHospital.json", params).then(function(data) {
					that.hospitalList = that.hospitalList.concat(data.hospitalList)
				})
			},
			getSectionList(){
				let that = this;
